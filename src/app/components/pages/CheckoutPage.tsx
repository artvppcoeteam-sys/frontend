...
export function CheckoutPage({ onNavigate }: CheckoutPageProps) {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'razorpay') {
      ...
      const options = {
        key: 'rzp_test_ARTVPPTESTKEY', // Official Test Key Placeholder
        ...
        <div className="mb-12">
          <div className="flex items-center justify-center gap-4">
            <div className="flex items-center group">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${step === 'shipping' ? 'bg-[#D4AF37] text-white shadow-lg scale-110' : 'bg-green-500 text-white'
                }`}>
                {step === 'shipping' ? '1' : <Check className="w-6 h-6" />}
              </div>
              <span className={`ml-3 hidden sm:inline font-medium ${step === 'shipping' ? 'text-[#D4AF37]' : 'text-green-600'}`}>Shipping</span>
            </div>
            <div className="w-24 h-0.5 bg-gray-200"></div>
            <div className="flex items-center group">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${step === 'payment' ? 'bg-[#D4AF37] text-white shadow-lg scale-110' : 'bg-gray-200 text-gray-500'
                }`}>
                2
              </div>
              <span className={`ml-3 hidden sm:inline font-medium ${step === 'payment' ? 'text-[#D4AF37]' : 'text-gray-400'}`}>Payment</span>
            </div>
          </div>
        </div>
                      ...
                      <form onSubmit={handleShippingSubmit} className="space-y-6">
                        ...
                        <div className="pt-4">
                          <Button
                            type="submit"
                            size="lg"
                            className="w-full bg-[#D4AF37] hover:bg-[#C19B2A] text-white py-6 text-lg rounded-full shadow-lg"
                          >
                            Continue to Payment
                          </Button>
                        </div>
                      ...
                      <form onSubmit={handlePaymentSubmit} className="space-y-8">
                        <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod}>
                          <div className="space-y-4">
                            {paymentOptions.map((option) => (
                              ...
                              >
                                <RadioGroupItem value={option.id} id={option.id} className="text-[#D4AF37]" />
                                <div className="flex items-center gap-4 flex-1">
                                  <div className={`p-3 rounded-lg ${paymentMethod === option.id ? 'bg-[#D4AF37] text-white' : 'bg-gray-100 text-gray-500'}`}>
                                    {option.icon}
                                  </div>
                                  <div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                          ...
                          <Button
                            type="submit"
                            size="lg"
                            className="flex-1 bg-[#D4AF37] hover:bg-[#C19B2A] text-white py-6 text-lg rounded-full shadow-lg"
                            disabled={isProcessing}
                          >
                  ...
                  {cart.map((item) => (
                    <div key={item.id} className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-semibold text-[#D4AF37] mt-1">₹{(item.price * item.quantity).toLocaleString()}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-4 pt-6 border-t font-medium">
                  ...
                  <div className="border-t pt-4 mt-4">
                    <div className="flex justify-between items-center text-gray-900">
                      ...
                      <span className="text-3xl font-bold text-[#D4AF37]">₹{total.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
